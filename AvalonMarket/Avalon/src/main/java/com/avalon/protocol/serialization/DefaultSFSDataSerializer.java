...
import com.avalon.core.data.AVAArray;
import com.avalon.core.data.AVAArrayLite;
import com.avalon.core.data.AVAObject;
import com.avalon.core.data.AVAObjectLite;
import com.avalon.core.data.DataType;
import com.avalon.core.data.DataWrapper;
import com.avalon.core.data.IArray;
import com.avalon.core.data.IObject;
import com.avalon.exception.SFSCodecException;
import com.avalon.exception.SFSRuntimeException;
...
public class DefaultSFSDataSerializer implements IDataSerializer
{
	...
	{
		AVAArray sfsArray = AVAArray.newInstance();
		byte headerBuffer = buffer.get();
		if (headerBuffer != DataType.SFS_ARRAY.getTypeID())
		{
			throw new IllegalStateException(
					"Invalid DataType. Expected: " + DataType.SFS_ARRAY.getTypeID() + ", found: " + headerBuffer);
		}
		else
		{
	...
	{
		AVAObject sfsObject = AVAObject.newInstance();
		byte headerBuffer = buffer.get();
		if (headerBuffer != DataType.SFS_OBJECT.getTypeID())
		{
			throw new IllegalStateException(
					"Invalid DataType. Expected: " + DataType.SFS_OBJECT.getTypeID() + ", found: " + headerBuffer);
		}
		else
		{
			short size = buffer.getShort();
	...
	{
		AVAArrayLite sfsArray = AVAArrayLite.newInstance();
	...
	{
		AVAObject sfsObject = AVAObjectLite.newInstance();
	...
	private DataWrapper decodeJsonObject(Object o)
	{
		...
		else if (o instanceof JSONObject)
		{
			...
			return jso.isEmpty()
					? new DataWrapper(DataType.NULL, (Object) null)
					: new DataWrapper(DataType.SFS_OBJECT, this.decodeSFSObject((JSONObject) jso));
		}
		else if (o instanceof JSONArray)
		{
			return new DataWrapper(DataType.SFS_ARRAY, this.decodeSFSArray((JSONArray) ((JSONArray) o)));
		}
		else
		{
			throw new IllegalArgumentException(String.format(
					"Unrecognized DataType while converting JSONObject 2 AVAObject. Object: %s, Type: %s", new Object[]
	...
	public byte[] object2binary(IObject object)
	{
		...
		buffer.put((byte) DataType.SFS_OBJECT.getTypeID());
	...
	{
		ByteBuffer buffer = ByteBuffer.allocate(BUFFER_CHUNK_SIZE);
		buffer.put((byte) DataType.SFS_ARRAY.getTypeID());
	...
	public void flattenObject(Map<String, Object> map, AVAObject sfsObj)
	{
		...
		while (it.hasNext())
		{
			...
			if (value.getTypeId() == DataType.SFS_OBJECT)
			{
				HashMap<String, Object> newList = new HashMap<String, Object>();
				map.put(key, newList);
				this.flattenObject(newList, (AVAObject) value.getObject());
			}
			else if (value.getTypeId() == DataType.SFS_ARRAY)
			{
				ArrayList<Object> newList1 = new ArrayList<Object>();
				map.put(key, newList1);
				this.flattenArray(newList1, (AVAArray) value.getObject());
			}
			else
			{
	...
	public void flattenArray(List<Object> array, AVAArray sfsArray)
	{
	...
	private DataWrapper decodeObject(ByteBuffer buffer) throws SFSCodecException
	{
		...
		else if (headerByte == DataType.SFS_ARRAY.getTypeID())
		{
			buffer.position(buffer.position() - 1);
			decodedObject = new DataWrapper(DataType.SFS_ARRAY, this.decodeSFSArray((ByteBuffer) buffer));
		}
		else
		{
			if (headerByte != DataType.SFS_OBJECT.getTypeID())
			{
				throw new SFSCodecException("Unknow DataType ID: " + headerByte);
			}

			buffer.position(buffer.position() - 1);
			IObject sfsObj = this.decodeSFSObject((ByteBuffer) buffer);
			DataType type = DataType.SFS_OBJECT;
	...
	private ByteBuffer encodeObject(ByteBuffer buffer, DataType typeId, Object object)
	{
		switch (typeId.ordinal())
		{
			...
			case 17 :
				...
				break;
			case 18 :
				buffer = this.addData(buffer, this.array2binary((AVAArray) object));
				break;
			case 19 :
				buffer = this.addData(buffer, this.object2binary((AVAObject) object));
				break;
			case 20 :
	...
	private DataWrapper binDecode_BOOL(ByteBuffer buffer) throws SFSCodecException
	{
	...
	private DataWrapper binDecode_UTF_STRING(ByteBuffer buffer) throws SFSCodecException
	{
	...
	private DataWrapper binDecode_BOOL_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
	...
	private DataWrapper binDecode_BYTE_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
	...
	private DataWrapper binDecode_SHORT_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
		short arraySize = this.getTypeArraySize(buffer);
		ArrayList array = new ArrayList();
	...
	private DataWrapper binDecode_INT_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
		short arraySize = this.getTypeArraySize(buffer);
		ArrayList array = new ArrayList();
	...
	private DataWrapper binDecode_LONG_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
		short arraySize = this.getTypeArraySize(buffer);
		ArrayList array = new ArrayList();
	...
	private DataWrapper binDecode_FLOAT_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
		short arraySize = this.getTypeArraySize(buffer);
		ArrayList array = new ArrayList();
	...
	private DataWrapper binDecode_DOUBLE_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
		short arraySize = this.getTypeArraySize(buffer);
		ArrayList array = new ArrayList();
	...
	private DataWrapper binDecode_UTF_STRING_ARRAY(ByteBuffer buffer) throws SFSCodecException
	{
		short arraySize = this.getTypeArraySize(buffer);
		ArrayList array = new ArrayList();
	...
	{
		byte[] data = new byte[]
		{(byte) DataType.BOOL.getTypeID(), (byte) (value.booleanValue() ? 1 : 0)};
	...
	{
		byte[] data = new byte[]
		{(byte) DataType.BYTE.getTypeID(), value.byteValue()};
		return this.addData(buffer, data);
	}
	...
	public IObject pojo2sfs(Object pojo)
	{
		AVAObject sfsObj = AVAObject.newInstance();
			...
			throws IllegalAccessException, NoSuchMethodException, IllegalArgumentException
	{
		...
		{
			AVAArray fieldList = AVAArray.newInstance();
			...
			for (int var7 = 0; var7 < var8; ++var7)
			{
				...
				try
				{
					...
					if (!Modifier.isTransient(err) && !Modifier.isStatic(err))
					{
						...
						AVAObject fieldDescriptor = AVAObject.newInstance();
	...
	private Object readValueFromGetter(String fieldName, String type, Object pojo) throws Exception
	{
		...
		String getterName = isBool
				? "is" + StringUtils.capitalize(fieldName)
	...
	private DataWrapper wrapPojoField(Object value)
	{
		...
		else
		{
			...
			{
				wrapper = new DataWrapper(DataType.SFS_ARRAY, this.unrollArray((Object[]) value));
			}
			else if (value instanceof Collection)
			{
				wrapper = new DataWrapper(DataType.SFS_ARRAY, this.unrollCollection((Collection) value));
			}
			else if (value instanceof Map)
			{
				wrapper = new DataWrapper(DataType.SFS_OBJECT, this.unrollMap( (Map) value));
			}
			else if (value instanceof SerializableSFSType)
			{
				wrapper = new DataWrapper(DataType.SFS_OBJECT, this.pojo2sfs(value));
			}
	...
	{
		AVAArray array = AVAArray.newInstance();
		Object[] var6 = arr;
		int var5 = arr.length;
	...
	public Object sfs2pojo(IObject sfsObj)
	{
		Object pojo = null;
		if (!sfsObj.containsKey(CLASS_MARKER_KEY) && !sfsObj.containsKey(CLASS_FIELDS_KEY))
		{
			throw new SFSRuntimeException("The AVAObject passed does not represent any serialized class.");
		}
		else
		{
			try
	...
	private void convertSFSObject(IArray fieldList, Object pojo) throws Exception
	{
		...
		{
			IObject fieldDescriptor = fieldList.getSFSObject(j);
	...
	private void writeValueFromSetter(Field field, Object pojo, Object fieldValue) throws Exception
	{
		...
		try
		{
			Method setterMethod = pojo.getClass().getMethod(setterName, new Class[]
			{field.getType()});
			setterMethod.invoke(pojo, new Object[]
			{fieldValue});
		}
	...
	private Object unwrapPojoField(DataWrapper wrapper)
	{
		...
		else if (type == DataType.SFS_ARRAY)
		{
			obj = this.rebuildArray((IArray) wrapper.getObject());
		}
		else if (type == DataType.SFS_OBJECT)
		{
